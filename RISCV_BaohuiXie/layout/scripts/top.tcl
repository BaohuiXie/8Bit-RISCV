...
# riscv8bit. The P&R flow is based on the EDI 13.1 official workshop and use the
...
# Load IO assignment file: riscv8bit.ioc
...
# Add power ring: use top metals (M9_output & M10_input) for power ring and set the ring
# width to be 1.5 and spacing to be 0.9, 
addRing -center 1 -type core_rings -width 1.5 -spacing 0.9 -nets {VDD VSS} \
        -layer {bottom metal10 right metal9 top metal10 left metal10}
...
# starting from the left offset 20.0 (roughly middle of the riscv8bit module)
...
verifyGeometry -report reports/riscv8bit.geometry.rpt
...
# *.gds: GDS file
streamOut results/riscv8bit.gds -mapFile streamOut.map \
